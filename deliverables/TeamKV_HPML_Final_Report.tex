...
H100 80\,GB GPU.  We present a single-codebase comparison of five
KV-cache optimization techniques spanning four compression families
(quantization via KIVI 2/4-bit, sparse dynamic selection via TopK /
TokenSelect, one-shot eviction via SnapKV, and latent projection via
...
are attributable to the KV policy only.  Our main results: KIVI 4-bit
incurs no measurable LongBench quality drop (0.292 vs.\ 0.291) while
delivering a 2.3$\times$ smaller KV cache and 1.93$\times$ decode
throughput at BS\,=\,32, with maximum single-GPU throughput rising
3.1$\times$ (1{,}517 vs.\ 497\,tok/s) once the larger batch ceiling is
exploited.  SnapKV marginally \emph{improves} overall quality
(+0.3\%) with zero per-step overhead.  TopK K\,=\,1024 matches
baseline quality and gains +20\% on TriviaQA by suppressing
distractor passages.  MLA achieves a consistent 3.9$\times$ KV
reduction and 1.3$\times$ decode speedup at batch size 1, but the
available checkpoint exhibits a large quality regression that requires
retraining at longer context.  No single method is best on all three
metrics, and the right choice depends on whether batch size, context
length, or output fidelity is the binding constraint.
\end{abstract}
...
Per-task patterns show two opposite breakdown mechanisms:
\emph{KIVI 2-bit and TopK K=512 both converge to overall score
$\approx$0.274 via different paths.}  KIVI's quantization noise
...
KIVI$\times$TopK 4b &  —   & 0.60$\times$ & 25.8\,GB & −51\% \\
\bottomrule
\end{tabular}
\vspace{1pt}
\footnotesize{Baseline OOMs at 64K; TopK-Flash succeeds at 65.8\,GB.  $^{\dagger}$SnapKV peak memory at 32K is interpolated from short-context measurements; 32K SnapKV throughput not directly benchmarked.}
\end{table}
...
decode speed drops 40\% (21.4$\rightarrow$12.9\,tok/s).  The slowdown
...
and per-step overhead must be zero.  Its 2.5$\times$ KV reduction adds
no per-step decode cost because the compressed cache is built once
after prefill.
...
throughput at BS=32, making it our recommended default for
batch-oriented serving.  SnapKV marginally improves LongBench
