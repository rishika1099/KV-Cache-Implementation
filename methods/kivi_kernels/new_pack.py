...
def unpack_and_dequant_kcache(k_code, scale, mn, group_size, bits):
    pack_dim = 2
...
def unpack_and_dequant_vcache(v_code, scale, mn, group_size, bits):
    assert bits in [2, 4, 8]
...
def unpack_tensor(v_code, bits, pack_dim):
    assert bits in [2, 4, 8]
    shape = v_code.shape
    feat_per_int = 32 // bits
