...
class SnapKVMethod(MethodWrapper):
    ...
    @property
    def true_seq_length(self):
        # Exposes the real absolute position for RoPE correction in runner.py.
        # After prefill eviction the DynamicCache is shorter than the original
        # prompt, so without this the model applies RoPE at the wrong position
        # and generates repetitive garbage.
        if self._original_seq_len is None:
            return None
        return self._original_seq_len + self._decode_steps
    ...
    def process_prefill(self, past_key_values, attention_weights=None):
        ...
        # Record original seq_len before any eviction for RoPE correction.
        self._original_seq_len = past_key_values[0][0].shape[2]
        self._decode_steps = 0
    ...
    def process_step(self, past_key_values, step, attention_weights=None):
        ...
        Increment the step counter so true_seq_length stays accurate.
        """
