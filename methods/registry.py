...
METHOD_REGISTRY: Dict[str, Callable[..., MethodWrapper]] = {
    ...
    "kivi":               _make_kivi,
    "topk":               _make_topk,
    "kivi_topk":          _make_kivi_topk,
    "kivi_topk_c":        _make_kivi_topk_c,
...
def describe(name: str) -> Dict[str, Any]:
    ...
    descriptions = {
        ...
        "kivi":         "KIVI asymmetric block-wise quantisation",
        "topk":         "TokenSelect dynamic top-K selection (Triton)",
        "kivi_topk":    "KIVI storage + centroid-scored top-K (design a)",
        "kivi_topk_c":  "KIVI storage + Triton quant-aware top-K (design c)",
        "kivi_topk_maxpool": "Hybrid with per-channel maxpool block summary",
        "kivi_topk_twopass": "Hybrid with centroid-then-exact two-pass selection",
        "kivi_topk_multiq":  "Hybrid with multi-query proxy over last 8 K-vectors",
        "kivi_topk_dynsink": "Hybrid with importance-picked sinks at prefill",
        "kivi_topk_all":     "Hybrid with maxpool + two-pass + multi-query + dyn-sinks",
    }
